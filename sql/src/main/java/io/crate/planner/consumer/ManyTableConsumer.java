        ...
        while (it.hasNext()) {
            ...
            if (it.hasNext()) { // The outer left join becomes the root {@link TwoTableJoin}
                ...
                Function<? super Symbol, ? extends Symbol> replaceFunction = FieldReplacer.bind(f -> {
                    // FIXME: this is a bit of a mess and needs explanation or a better approach
                    if (f.relation() == left || f.relation() == right) {
                        return Objects.requireNonNull(
                            join.getField(new ColumnIdent(f.relation().getQualifiedName().toString(), f.path().outputName()), Operation.READ),
                            "must be able to resolve the field from the twoTableJoin");
                    }
                    return f;
                });
